...
- Slide Spec 新增场景、受众、工作流模式、高分模式、生成控制、证据状态、字段锁定和局部修改契约。
- 新增分层内容工作流，固定“目录 → 每页目标 → 页面要点 → PPT 文案 → 演讲稿”的生成顺序。
- 新增 `scripts/analyze_slide_spec.py`，检测重复/空泛标题、缺少证据、待核验来源、缺少转场、缺少收束和计划超时。
- Review 增加逐页五项评分与演练检查；PPTX 生产必须遵守锁定字段和局部修改边界。
- 新增 schema、语义验证、质量分析和行为契约测试。
