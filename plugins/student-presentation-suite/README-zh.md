...
本目录是插件本体目录。在本仓库中，marketplace 文件放在仓库根目录：
...
把 marketplace 仓库复制或克隆到本地 Codex 插件 marketplace 目录，然后重启或刷新 Codex 插件发现。
...
如果按上面的命令克隆到 `$env:USERPROFILE\.agents\plugins`，个人 marketplace 入口 `$env:USERPROFILE\.agents\plugins\marketplace.json` 中该插件的 `source.path` 应指向插件目录：

```json
"./plugins/student-presentation-suite"
```

这个 marketplace 文件应保留在仓库根目录，而不是放进当前插件目录。
